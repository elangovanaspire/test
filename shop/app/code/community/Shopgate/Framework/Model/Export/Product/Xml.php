    ...
    extends Shopgate_Model_Catalog_Product
{
    ...
    public function setPrice()
    {
        ...
        if (!$this->getIsChild() || ($this->getIsChild() && $this->_parent->isGrouped()) || ($this->getIsChild() 
            && !Mage::getStoreConfig(Shopgate_Framework_Model_Config::XML_PATH_SHOPGATE_EXPORT_USE_ROOT_PRICES))) {
            $price->setPrice($this->_formatPrice($this->item->getPrice()));
            $price->setCost($this->_formatPrice($this->item->getCost()));
	        ...
	        if (Mage::getStoreConfig(Shopgate_Framework_Model_Config::XML_PATH_SHOPGATE_EXPORT_USE_PRICE_INDEX_ON_EXPORT)) {
				...
				if ($this->item->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE) {
					$finalPrice = $index->load($this->item->getId())->getMinPrice();
				} else {
					$finalPrice = $index->load($this->item->getId())->getFinalPrice();	
				}
	        } else {
	        ...
	        if ($this->item->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE
		        ...
		        $price->setPrice($minimalPrice);
	        }

	        $price->setSalePrice($this->_formatPrice($finalPrice));
	        $price->setMsrp($this->_formatPrice($this->item->getMsrp()));
            ...
            $this->_createTierPriceNode($price);
            $this->_createGroupPriceNode($price);
        }
